...
namespace DualPantoToolkit
{
    ...
    /// and spawns solid Wall cubes (PantoBoxCollider) along every border edge.
    ///
    /// Algorithm:
    ...
    ///   6. Spawn a Wall cube for each edge.
    ///
    ...
    /// </summary>
    [ExecuteAlways]
    public class WallOutlineGenerator : MonoBehaviour
    {
        // ── Inspector ─────────────────────────────────────────────────────────
        [Header("Wall Settings")]
        [Tooltip("Thickness (world units) of each spawned Wall cube — the narrow side.")]
        public float wallThickness = 0.05f;

        [Tooltip("Height (world units) of each spawned Wall cube.")]
        public float wallHeight = 0.1f;

        [Tooltip("Y position (world space) at which Wall cubes are placed.")]
        public float wallY = 0f;

        [Tooltip("Parent transform for the generated Walls. Defaults to this transform.")]
        public Transform wallParent;

        [Tooltip("If true, also spawn Walls along inner hole boundaries " +
        ...
        public void Clear()
        {
            foreach (var go in _spawnedWalls)
        ...
        private void CollectPolygons()
        {
            ...
            Debug.Log($"[WallOutlineGenerator] Collected {_inputPolygons.Count} polygon(s).");
        }
        ...
        private void BuildOutlines()
        {
            ...
            Debug.Log($"[WallOutlineGenerator] {_outerOutlines.Count} outer outline(s), " +
        ...
        private void SpawnWalls()
        {
            ...
            foreach (var outline in _outerOutlines)
                SpawnWallsForOutline(outline, parent, "Wall_Outer");

            if (generateInline)
                foreach (var hole in _innerHoles)
                    SpawnWallsForOutline(hole, parent, "Wall_Inner");

            Debug.Log($"[WallOutlineGenerator] Spawned {_spawnedWalls.Count} Wall(s).");
        }

        private void SpawnWallsForOutline(List<Vector2> poly, Transform parent, string prefix)
        {
            int n = poly.Count;
            for (int i = 0; i < n; i++)
            {
                ...
                SpawnEdgeWall(a2, b2, parent, prefix);
            }
        }

        private void SpawnEdgeWall(Vector2 a2, Vector2 b2, Transform parent, string namePrefix)
        {
            float length = Vector2.Distance(a2, b2);
            if (length < 0.001f) return;

            Vector3 a   = new Vector3(a2.x, wallY, a2.y);
            Vector3 b   = new Vector3(b2.x, wallY, b2.y);
            Vector3 mid = (a + b) * 0.5f;
            Vector3 dir = (b - a).normalized;

            var go = GameObject.CreatePrimitive(PrimitiveType.Cube);
            go.name = $"{namePrefix}_{_spawnedWalls.Count:D3}";
            ...
            var wall = go.AddComponent<PantoBoxCollider>();
...
// ═════════════════════════════════════════════════════════════════════════════
#if UNITY_EDITOR
    [CustomEditor(typeof(WallOutlineGenerator))]
    public class WallOutlineGeneratorEditor : Editor
    {
        public override void OnInspectorGUI()
        {
            DrawDefaultInspector();
            EditorGUILayout.Space(6);
            var gen = (WallOutlineGenerator)target;
            using (new EditorGUI.DisabledScope(!Application.isPlaying && false)) // always enabled
            {
                if (GUILayout.Button("▶  Generate Walls", GUILayout.Height(28))) gen.Generate();
                if (GUILayout.Button("✕  Clear Walls",    GUILayout.Height(24))) gen.Clear();
            }
        }
    }
#endif
